...
Message Property TTT_ChooseNewCardsMsg Auto
...
Int Property MaxBet = 0 Auto Hidden
...
; Included by TTTTE_v1 Game Board Variety Selection
Message Property TT_SelectBoardMsg Auto 

Light Property TT_BoardTorch Auto
Light Property TT_BoardValenwood Auto
Light Property TT_BoardStormcloak Auto
Light Property TT_BoardRustic Auto
Light Property TT_BoardRift Auto
Light Property TT_BoardImperial Auto
Light Property TT_BoardRed Auto
Light Property TT_BoardAldmeri Auto
Light Property TT_BoardBrotherhood Auto

ObjectReference Property TT_BoardDefault_DISP Auto
ObjectReference Property TT_BoardValenwood_DISP Auto
ObjectReference Property TT_BoardStormcloak_DISP Auto 
ObjectReference Property TT_BoardRustic_DISP Auto 
ObjectReference Property TT_BoardRift_DISP Auto
ObjectReference Property TT_BoardImperial_DISP Auto 
ObjectReference Property TT_BoardRed_DISP Auto
ObjectReference Property TT_BoardAldmeri_DISP Auto
ObjectReference Property TT_BoardBrotherhood_DISP Auto

ObjectReference Property SelectedGameBoard Auto Hidden
;END Included by TTTTE_v1 Game Board Variety Selection
...
;END Included by TTTT_v1
    ...
    ; Detect if player has multiple boards, if so, ask which one to use.
    If BoardsInPossession() > 1
        Debug.Notification("Multiple Boards Detected")
        ; Disable all boards immediately after setup
        TT_BoardDefault_DISP.Disable()
        TT_BoardValenwood_DISP.Disable()
        TT_BoardStormcloak_DISP.Disable()
        TT_BoardRustic_DISP.Disable()
        TT_BoardRift_DISP.Disable()
        TT_BoardImperial_DISP.Disable()
        TT_BoardRed_DISP.Disable()
        TT_BoardAldmeri_DISP.Disable()
        TT_BoardBrotherhood_DISP.Disable()
        wait(0.1)

        ; Ask user which board they would like to use
        Int selectedBoard = TT_SelectBoardMsg.Show()
        wait(0.1)

        If selectedBoard == 0 ; Valenwood
            Debug.Notification("Using Valenwood Board")
            SelectedGameBoard = TT_BoardValenwood_DISP
            TT_BoardValenwood_DISP.Enable()
        ElseIf selectedBoard == 1 ; Stormcloak
            Debug.Notification("Using Stormcloak Board")
            SelectedGameBoard = TT_BoardStormcloak_DISP
            TT_BoardStormcloak_DISP.Enable()
        ElseIf selectedBoard == 2 ; Rustic
            Debug.Notification("Using Rustic Board")
            SelectedGameBoard = TT_BoardRustic_DISP
            TT_BoardRustic_DISP.Enable()
        ElseIf selectedBoard == 3 ; Rift
            Debug.Notification("Using Rift Board")
            SelectedGameBoard = TT_BoardRift_DISP
            TT_BoardRift_DISP.Enable()
        ElseIf selectedBoard == 4 ; Imperial
            Debug.Notification("Using Imperial Board")
            SelectedGameBoard = TT_BoardImperial_DISP
            TT_BoardImperial_DISP.Enable()
        ElseIf selectedBoard == 5 ; Basic Red
            Debug.Notification("Using Basic Red Board")
            SelectedGameBoard = TT_BoardRed_DISP
            TT_BoardRed_DISP.Enable()
        ElseIf selectedBoard == 6 ; Aldmeri
            Debug.Notification("Using Aldmeri Board")
            SelectedGameBoard = TT_BoardAldmeri_DISP
            TT_BoardAldmeri_DISP.Enable()
        ElseIf selectedBoard == 6 ; Brotherhood
            Debug.Notification("Using Brotherhood Board")
            SelectedGameBoard = TT_BoardBrotherhood_DISP
            TT_BoardBrotherhood_DISP.Enable()
        Else ; Default
            SelectedGameBoard = TT_BoardDefault_DISP
            TT_BoardDefault_DISP.Enable()
        EndIf
    Else ; Player only has one board in their inventory
        Debug.Notification("Only a single board detected")
        ; Disable all boards immediately after setup
        TT_BoardDefault_DISP.Disable()
        TT_BoardValenwood_DISP.Disable()
        TT_BoardStormcloak_DISP.Disable()
        ...
        ; Check the SelectedGameBoard for the board the player has in their inventory
        ; @NOTE SelectedGameBoard is set during the BoardsInPossession() check
        If SelectedGameBoard == TT_BoardValenwood_DISP
            TT_BoardValenwood_DISP.Enable()
        ElseIf SelectedGameBoard == TT_BoardStormcloak_DISP
            TT_BoardStormcloak_DISP.Enable()
        ElseIf SelectedGameBoard == TT_BoardRustic_DISP
            TT_BoardRustic_DISP.Enable()
        ElseIf SelectedGameBoard == TT_BoardRift_DISP
            TT_BoardRift_DISP.Enable()
        ElseIf SelectedGameBoard == TT_BoardImperial_DISP
            TT_BoardImperial_DISP.Enable()
        ElseIf SelectedGameBoard == TT_BoardRed_DISP
            TT_BoardRed_DISP.Enable()
        ElseIf SelectedGameBoard == TT_BoardAldmeri_DISP
           TT_BoardAldmeri_DISP.Enable()
        ElseIf SelectedGameBoard == TT_BoardBrotherhood_DISP
            TT_BoardBrotherhood_DISP.Enable()
        Else ; Default
            SelectedGameBoard = TT_BoardDefault_DISP
            TT_BoardDefault_DISP.Enable()
    ...
    ; Confirm Game Rules with Player
    SetGameRules()
    Wait(0.1)
...
Int Function BoardsInPossession()
    Int boardCount = 0

    Int TT_BoardTorchCount = PlayerRef.GetItemCount(TT_BoardTorch)
    Int TT_BoardValenwoodCount = PlayerRef.GetItemCount(TT_BoardValenwood)
    Int TT_BoardStormcloakCount = PlayerRef.GetItemCount(TT_BoardStormcloak)
    Int TT_BoardRusticCount = PlayerRef.GetItemCount(TT_BoardRustic)
    Int TT_BoardRiftCount = PlayerRef.GetItemCount(TT_BoardRift)
    Int TT_BoardImperialCount = PlayerRef.GetItemCount(TT_BoardImperial)
    Int TT_BoardRedCount = PlayerRef.GetItemCount(TT_BoardRed)
    Int TT_BoardAldmeriCount = PlayerRef.GetItemCount(TT_BoardAldmeri)
    Int TT_BoardBrotherhoodCount = PlayerRef.GetItemCount(TT_BoardBrotherhood)
    
    boardCount += TT_BoardTorchCount
    boardCount += TT_BoardValenwoodCount
    boardCount += TT_BoardStormcloakCount
    boardCount += TT_BoardRusticCount
    boardCount += TT_BoardRiftCount
    boardCount += TT_BoardImperialCount
    boardCount += TT_BoardRedCount
    boardCount += TT_BoardAldmeriCount
    boardCount += TT_BoardBrotherhoodCount
    Debug.Notification("Player is carrying " + boardCount as String + " Boards")

    ; If player is only carrying a single board
    If boardCount <= 1
        If TT_BoardValenwoodCount >= 1
            Debug.Notification("Player has the Valenwood Board")
            SelectedGameBoard = TT_BoardValenwood_DISP
        ElseIf TT_BoardStormcloakCount >= 1
            Debug.Notification("Player has the Stormcloak Board")
            SelectedGameBoard = TT_BoardStormcloak_DISP
        ElseIf TT_BoardRusticCount >= 1
            Debug.Notification("Player has the Rustic Board")
            SelectedGameBoard = TT_BoardRustic_DISP
        ElseIf TT_BoardRiftCount >= 1
            Debug.Notification("Player has the Rift Board")
            SelectedGameBoard = TT_BoardRift_DISP
        ElseIf TT_BoardImperialCount >= 1
            Debug.Notification("Player has the Imperial Board")
            SelectedGameBoard = TT_BoardImperial_DISP
        ElseIf TT_BoardRedCount >= 1
            Debug.Notification("Player has the Red Board")
            SelectedGameBoard = TT_BoardRed_DISP
        ElseIf TT_BoardAldmeriCount >= 1
            Debug.Notification("Player has the Aldmeri Board")
            SelectedGameBoard = TT_BoardAldmeri_DISP
        ElseIf TT_BoardBrotherhoodCount >= 1
            Debug.Notification("Player has the Brotherhood Board")
            SelectedGameBoard = TT_BoardBrotherhood_DISP
        Else
            Debug.Notification("Player has the Default Board")
            SelectedGameBoard = TT_BoardDefault_DISP
        EndIf
    EndIf
...
;END Included by TTTT_v1
    ...
    ; Debug.Notification("Player gold = " + PlayerGold)
    ; Debug.Notification("Opponent gold = " + OpponentGold)
    ; Debug.Notification("Prefered Bet Amount = " + PreferedBetAmount)
    ...
    If PlayerGold >= 10 && OpponentGold >= 10 && PreferedBetAmount > 0
        ...
        If TT_SetBetConfirmMsg.Show() == 0 || TT_SetBetConfirmMsg.Show() == 5
            ...
            ; Set Max Bet Amount
            MaxBet = PlayerGold
            If OpponentGold < MaxBet
                MaxBet = OpponentGold
            EndIf

            ;makes max bet amount only multiples of 10
            Float MaxBetTenCheck = ((MaxBet) / 10.0)
            Float MaxBetTenCheckDiff = ((MaxBetTenCheck - (Math.Floor(MaxBetTenCheck) as float)) * 10)
            
            Float NewMaxBet = (MaxBet as float) - MaxBetTenCheckDiff
            MaxBet = NewMaxBet as int
            Debug.Notification("Max Bet = " + MaxBet)
...
;TOP Included by TTTT_v1
		...
		If cards > 0
			...
			choice = TTT_ChooseNewCardsMsg.Show()
		EndIf
...
Function OpponentPlacesCard()
    ...
    If CardsPlaced > 0
        ...
        While Ia < 9
            ...
            If Ia == 5 && CardsPlaced < 8
                ;Debug.Notification("Thinking...")
                TT_ThinkingMsg.Show()
            Endif 
            
            If TT_BoardMarkers[Position].IsEnabled() 
                Int Ib = TT_OpponentCards.GetSize() 
                While Ib > 0
                    Ib -= 1 
                    MiscObject Card = TT_OpponentCards.GetAt(Ib) as MiscObject
                    Int CurrentCardsCanFlip = PlaceCard(Position, Card, 1, false, true)
                    Wait(0.1)
                    If CurrentCardsCanFlip > CardsCanFlip
                        CardsCanFlip = CurrentCardsCanFlip
...
Function EndGame()
    ...
    Debug.Notification("End Game Gift Menu Debug: Line 2911")
    ...
    Elseif Score[0] > Score[1]
        ...
        Debug.Notification("Player Won Gift Menu Debug: Line 2940")
        ...
        If TradeRules == 0 || TradeRules == 2 
            ...
            Debug.Notification("SetUpForGiftMenu Gift Menu Debug: Line 2953")
            SetUpForGiftMenu()
            ...
            TransferCards = True
            Wait(0.2) 
            Debug.Notification("Showing Gift Menu Gift Menu Debug: Line 2963")
            Opponent.ShowGiftMenu(false, TT_OpponentCards, true, false)
            Wait(0.1)
            If OpponentHasTT_CardToGet == true 
                Debug.Notification("Opp has cards to get Gift Menu Debug: Line 2967")
                ObjectReference CardToGet = TT_CardToGet.GetReference()
            ...
            Debug.Notification("Removing Opponents Cards Gift Menu Debug: Line 2977")
            ; Opponent.RemoveItem(TT_OpponentCards, 1)
            Debug.Notification("Attempting to close the Gift Menu: Line 2980")
            UI.InvokeString("HUD Menu", "_global.skse.CloseMenu", "GiftMenu")
            GiftMenuOpen = False
            TransferCards = False
            Debug.Notification("Card Game EndingB() Gift Menu Debug: Line 2984")
            EndgameB()
...
Function DirectTrade()
    If CancelGame == True || PlayerRef.GetParentCell() != TT_WinnerChestA.GetParentCell() 
        Return
    Endif 
    Debug.Notification("Direct Trade")
    ObjectReference CardToGet = TT_CardToGet.GetReference()
    Form CardToGetBase = CardToGet.GetBaseObject()
    ...
    Wait(0.2) 
    Debug.Notification("Attempting to close the Gift Menu and Container Menu: Line 3306")
    UI.InvokeString("HUD Menu", "_global.skse.CloseMenu", "GiftMenu")
...
Function AddUniqueCardsToLists()
    ...
    While M > 0 
        ...
        ;     Index = 8
        ; Endif
        ;END Excluded by TTTTE_v1

        ;Included by TTTTE_v1
        If Card.HasKeywordString("TT_Rarity_8")
            Index = 7
        Elseif Card.HasKeywordString("TT_Rarity_9")
            Index = 8
        Endif
        ;END Included by TTTTE_v1
